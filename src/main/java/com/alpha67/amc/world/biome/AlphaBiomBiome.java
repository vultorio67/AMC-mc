...
package com.alpha67.amc.world.biome;
...
public class AlphaBiomBiome extends AmcModElements.ModElement {
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(-16764109).setWaterFogColor(-16764109)
						.withSkyColor(7972607).withFoliageColor(-13434829).withGrassColor(-16777216).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.5f).scale(0.7f).temperature(0.5f)
						.downfall(0.7f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("amc:alpha_biom"));
			}
		}
	}
